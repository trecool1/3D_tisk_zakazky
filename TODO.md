...
- [x] Přetahování karty myší na desktopu — podle zpětné vazby („na tabletu se to ovládá mnohem líp“) mělo nativní HTML5 drag-and-drop s myší znatelně horší odezvu (poloprůhledný "duch" karty nesledoval kurzor přesně). Karta se teď pro myš táhne vlastní logikou (pointer events) věrně pod kurzorem, stejně plynule jako na dotyku (dlouhý stisk a tažení); dotyk zůstává na nativním DnD beze změny. Dropdown pro změnu stavu na kartě byl nechtěný — odstraněn, jediná cesta je tažení.
- [x] Posun tabule mezi sloupci — na dotyku šlo tabuli posouvat přirozeně tažením prstu, s myší jen kolečkem. Teď jde tabuli "chytit" i myší za prázdnou plochu (mimo kartu a ovládací prvky) a přetáhnout do stran stejně jako swipe na tabletu; kolečko zůstává funkční vedle toho. Nasazeno a ověřeno vedle přetahování karet (bez konfliktu).
